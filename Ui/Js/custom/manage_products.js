var productModal = $("#productModal");
            ...
            if(response) {
                ...
                $.each(response, function(index, product) {
                    table += '<tr data-id="'+ product.product_id +'" data-name="'+ product.name +'" data-unit="'+ product.unit_id +'" data-price="'+ product.price_per_unit +'">' +
                        ...
                        '<td><span class="btn btn-xs btn-danger delete-product">Delete</span> <button class="btn btn-xs btn-danger edit-product data-target="#productModelList"">Edit</button></td></tr>';
                });
    ...
    productModelList.on('hide.bs.modal', function(){
        $("#id").val('0');
        $("#name, #unit, #price").val('');
        productModal.find('.modal-title').text('Add New Product');
    });

    productModelList.on('show.bs.modal', function(){
        //JSON data by API call
        $.get(unitListApiUrl, function (response) {
            if(response) {
                var options = '<option value="">--Select--</option>';
